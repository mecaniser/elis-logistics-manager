...
def _is_77_cargo_settlement(text: str, settlement_type: str = None) -> bool:
    """Detect a 77 Cargo / ezLoads single-settlement layout.

    Text extraction from a PDF is not stable across PDF producers: columns can
    be merged, labels can wrap, and the ``Description`` column can disappear.
    Require the carrier and a settlement marker, rather than an exact copy of
    one particular table header.
    """
    if settlement_type and settlement_type.upper() == "77 CARGO LLC":
        return True

    return bool(
        re.search(r"\b77\s+Cargo(?:\s+LLC)?\b", text, re.IGNORECASE)
        and re.search(r"\bSettlement\s*#", text, re.IGNORECASE)
        and re.search(r"\bLoad\s*#", text, re.IGNORECASE)
    )
...
def _parse_short_date(date_str: str):
    for date_format in ("%m/%d/%y", "%m/%d/%Y", "%m-%d-%y", "%m-%d-%Y"):
        try:
            ...
            continue
    raise ValueError(f"Unsupported settlement date: {date_str}")
...
def _extract_77_cargo_section_subtotal(section: str):
    """Extract a negative subtotal from an ezLoads accounting section."""
    matches = re.findall(r"(?im)^\s*Subtotal\s*:?\s*-?\$?([\d,]+(?:\.\d{2})?)\s*$", section)
    return _parse_currency(matches[-1]) if matches else None


def _extract_77_cargo_load_rows(text: str) -> List[Dict[str, str]]:
    """Extract load rows from an ezLoads table without relying on column spacing."""
    header_match = re.search(r"(?im)^\s*Load\s*#.*$", text)
    if not header_match:
        raise ValueError("77 Cargo load section not found")

    # The next accounting-section heading is more reliable than the exact
    # spelling/spacing of the subtotal row in PDFs generated by ezLoads.
    section_end = re.search(
        r"(?im)^\s*(?:Fuel|Tolls|Additions|Deductions|Settlement\s+total|Balance\s+due)\b",
        text[header_match.end():],
    )
    load_section = text[header_match.end():]
    if section_end:
        load_section = load_section[:section_end.start()]

    date_pattern = r"\d{1,2}[/-]\d{1,2}[/-]\d{2,4}"
    load_row_pattern = re.compile(
        rf"^(?P<load_id>\d{{3,12}})\s+"
        rf"(?P<pickup>{date_pattern})\s+"
        rf"(?P<delivery>{date_pattern})"
        r"(?:\s+.*?)?\s+"
        r"(?P<empty_miles>\d+(?:\.\d+)?)\s+"
        r"(?P<loaded_miles>\d+(?:\.\d+)?)\s+"
        r"(?P<pay_percent>\d+(?:\.\d+)?)%?\s+"
        r"\$?(?P<rate_amount>[\d,]+(?:\.\d{2})?)\s+"
        r"\$?(?P<gross_amount>[\d,]+(?:\.\d{2})?)\s*$"
    )

    load_rows = []
    for raw_line in load_section.splitlines():
        line = raw_line.strip()
        if not line:
            continue
...
def _parse_77_cargo_pdf(text: str) -> Dict:
    ...
    settlement_date_match = re.search(
        r"(?im)^\s*Date\s*:\s*(\d{1,2}[/-]\d{1,2}[/-]\d{2,4})", text
    )
    ...
    for row in load_rows:
        ...
        miles_total += float(row["empty_miles"]) + float(row["loaded_miles"])
    ...
    miles_subtotal_match = re.search(
        r"(?im)^\s*Subtotal\s*:\s*(\d+(?:\.\d+)?)\s+(\d+(?:\.\d+)?)\s+\$?([\d,]+(?:\.\d{2})?)\s+\$?([\d,]+(?:\.\d{2})?)\s*$",
        text,
    )
    if miles_subtotal_match:
        settlement_data["miles_driven"] = float(miles_subtotal_match.group(1)) + float(miles_subtotal_match.group(2))
    ...
    if fuel_total is not None:
        expense_categories["fuel"] = fuel_total
    if tolls_total is not None:
        expense_categories["tolls"] = tolls_total

    # An ezLoads "Additions" section offsets deductions (for example, paying
    # out a prior negative balance).  It is cash coming into the settlement,
    # not current-period revenue or an expense.  Keep it separate so the
    # operating gross/expenses remain accurate while the net matches the PDF.
    reimbursement_details = []
    cash_adjustments = []
    if additions_total:
        addition_lines = []
        for raw_line in additions_section.splitlines():
            line = raw_line.strip()
            if not line or re.match(r"(?i)^(?:Load\s*#|#)?\s*Date\s+Description\s+Amount$", line):
                continue
            if re.match(r"(?i)^Subtotal\s*:?", line):
                continue
            if re.match(r"^-\s+\d{1,2}/\d{1,2}/\d{2,4}\s+\$?[\d,]+\.\d{2}$", line):
                continue
            if re.search(r"(?i)^(?:ezloads\s+tms|ezloads\.net)", line):
                continue
            addition_lines.append(line)

        description = " ".join(addition_lines).strip(" :-") or "Settlement addition"
        if "balance out" in description.lower() or "previous negative balance" in description.lower():
            cash_adjustments.append({
                "type": "prior_balance_offset",
                "description": description,
                "amount": round(additions_total, 2),
            })
        else:
            reimbursement_details.append({
                "description": description,
                "amount": round(additions_total, 2),
            })
            expense_categories["reimbursement"] = round(additions_total, 2)
            settlement_data["reimbursement_details"] = reimbursement_details
    ...
    if deductions_section and deductions_total is not None:
        # Drop table headings and the subtotal.  Keeping this line-oriented
        # makes the parser resilient when a description wraps onto its own
        # line or the PDF omits the '# Date Description Amount' heading.
        deductions_block = re.sub(r"(?im)^\s*(?:#\s*)?Date\s+Description\s+Amount\s*$", "", deductions_section)
        deductions_block = re.sub(r"(?im)^\s*Subtotal\s*:?.*$", "", deductions_block)
        deductions_subtotal = deductions_total

        # Some long descriptions wrap before their date and amount when
        # extracted from the PDF.  In the rendered table this is still one
        # deduction row; restore that row before applying the normal parser.
        # Example: ``Balance In ... Unit 609 -\n- 07/21/26 -$996.99``.
        deductions_block = re.sub(
            r"(?m)^(.+?\s+-)\s*\n\s*-\s+(\d{1,2}/\d{1,2}/\d{2,4})\s+(-\$[\d,]+\.\d{2})\s*$",
            r"- \2 \1 \3",
            deductions_block,
        )
        ...
        for _, description, amount_str in other_rows:
            ...
            if "balance in" in lower_description or "negative balance" in lower_description:
                # This is a carry-forward cash adjustment, not a cost incurred
                # by this load period.  Preserve its sign so the reported cash
                # payout can be reconciled without distorting operating profit.
                cash_adjustments.append({
                    "type": "prior_balance_offset",
                    "description": normalized_description,
                    "amount": round(-amount, 2),
                })
            elif "cargo and liability insurance" in lower_description:
    ...
    if cash_adjustments:
        settlement_data["cash_adjustments"] = cash_adjustments

    gross_revenue = round(gross_subtotal, 2)
    total_expenses = round(
        sum(amount for category, amount in expense_categories.items() if category != "reimbursement"),
        2,
    )
    reported_net = None
    ...
    operating_profit = round(gross_revenue - total_expenses, 2)
    cash_adjustment_total = round(sum(item["amount"] for item in cash_adjustments), 2)
    calculated_cash_settlement = round(
        operating_profit + expense_categories.get("reimbursement", 0.0) + cash_adjustment_total,
        2,
    )
    if reported_net is not None and abs(calculated_cash_settlement - reported_net) > 0.01:
        raise ValueError(
            f"77 Cargo cash settlement mismatch: calculated {calculated_cash_settlement:.2f} vs reported {reported_net:.2f}"
        )

    settlement_data["gross_revenue"] = gross_revenue
    settlement_data["expenses"] = total_expenses
    settlement_data["net_profit"] = operating_profit
    settlement_data["cash_settlement_amount"] = reported_net if reported_net is not None else calculated_cash_settlement
