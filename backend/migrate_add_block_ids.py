#!/usr/bin/env python3
"""
Migration script to add block_ids column to settlements table
"""
import sqlite3
import json
from pathlib import Path

# Get database path
db_path = Path(__file__).parent / "elisgroup.db"

if not db_path.exists():
    print(f"Database file not found at {db_path}")
    print("The database will be created automatically on next app startup.")
    exit(0)

print(f"Connecting to database: {db_path}")

conn = sqlite3.connect(db_path)
cursor = conn.cursor()

# Check if block_ids column already exists
cursor.execute("PRAGMA table_info(settlements)")
columns = [column[1] for column in cursor.fetchall()]

if 'block_ids' in columns:
    print("✓ Column 'block_ids' already exists in settlements table.")
else:
    print("Adding 'block_ids' column to settlements table...")
    try:
        # SQLite doesn't have native JSON type, so we use TEXT
        cursor.execute("ALTER TABLE settlements ADD COLUMN block_ids TEXT")
        conn.commit()
        print("✓ Successfully added 'block_ids' column to settlements table.")
    except sqlite3.Error as e:
        print(f"✗ Error adding column: {e}")
        conn.rollback()
        conn.close()
        exit(1)

conn.close()
print("\n✓ Migration completed successfully!")

