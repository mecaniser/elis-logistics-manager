...
export default function BankSelect({ value, options, onChange, ariaLabel, required = false }: {
  ...
  const openMenu = () => {
    ...
    const roomBelow = rect ? window.innerHeight - rect.bottom : 999
    setPlacement(rect && roomBelow < 210 && rect.top > roomBelow ? 'up' : 'down')
  ...
  return <div ref={root} className="relative mt-1 min-w-0" onKeyDown={onKeyDown}>
    ...
    {open && <div role="listbox" aria-label={ariaLabel} className={`absolute inset-x-0 z-50 max-h-64 overflow-auto rounded-xl border border-slate-200 bg-white p-1.5 shadow-[0_16px_40px_-12px_rgba(15,23,42,0.35)] ${placement === 'up' ? 'bottom-full mb-1 origin-bottom' : 'top-full mt-1 origin-top'}`}>
