...
export default function BankTransferQueue({ tenantId, checking, sources, basis, onAccountsDiscovered }: { tenantId: number; checking: Account[]; sources: Account[]; basis: string; onAccountsDiscovered: (accounts: Account[]) => void }) {
  ...
  const retryPreparation = async (draft: Draft) => {
    setBusy(true); setError(''); setNotice(''); setOperation({ message: 'Releasing the interrupted preparation…', started: Date.now() })
    try {
      await bankMonitorApi.retryDraft(tenantId,draft.id)
      await reload()
      if (active.current) setNotice('The interrupted draft is ready again. Sign in to Truliant, then select Prepare in Truliant.')
