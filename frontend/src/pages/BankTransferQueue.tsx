...
type Draft = { bank_session?: { profile_id: string; profile_name: string } | null; id: string; charge_reference: string; amount_cents: number; from_last4: string; to_last4: string; memo: string; kind: 'coverage' | 'repayment'; status: string; bank_date: string; bank_state?: 'posted' | 'pending' | null; bank_effective_date?: string | null }
...
const REQUIRED_EXTENSION_VERSION = '0.1.24'
...
const apiErrorMessage = (error: unknown, fallback: string) => {
  if (typeof error === 'object' && error && 'response' in error) {
    ...
    if (typeof response?.data?.detail === 'string') return response.data.detail.replace(/_/g, ' ')
  }
...
const bankDateIso = (value: string) => {
  const parsed = new Date(value)
  if (Number.isNaN(parsed.valueOf())) return null
  return `${parsed.getFullYear()}-${String(parsed.getMonth() + 1).padStart(2, '0')}-${String(parsed.getDate()).padStart(2, '0')}`
}
const bankDateLabel = (value: string | null | undefined) => value ? new Date(`${value}T12:00:00`).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : ''
...
function Icon({ name, className = 'h-5 w-5' }: { name: 'check' | 'alert' | 'refresh' | 'link' | 'arrow' | 'clock' | 'wallet' | 'credit'; className?: string }) {
  const paths = {
    ...
    wallet: <><path d="M4 6.5A2.5 2.5 0 0 1 6.5 4H18a2 2 0 0 1 2 2v12a2 2 0 0 1-2 2H6a3 3 0 0 1-3-3V7a3 3 0 0 1 3-3" /><path d="M16 12h4" /><circle cx="16" cy="12" r=".5" fill="currentColor" stroke="none" /></>,
...
const statusMeta = (status: string) => {
  ...
  if (status === 'preparation_failed') return { label: 'Preparation interrupted', tone: 'bg-amber-50 text-amber-800 ring-amber-200', action: 'retry' as const }
  return { label: status.replace(/_/g, ' '), tone: 'bg-slate-100 text-slate-700 ring-slate-200', action: 'blocked' as const }
}
const plaidMatchMessage = (match: PlaidMatch) => ({
  feed_not_ready: 'Plaid is still preparing transaction history. Check again after its next update.',
  posting_pending: 'At least one side is still pending or missing. Wait for both entries to post before confirming.',
  not_found: 'Neither exact posted pair is available in Plaid yet. No completion was assumed.',
  ambiguous: 'More than one entry has this amount. Review the transfer in Truliant before marking it complete.',
  needs_bank_review: 'Plaid found two entries, but could not identify them as a transfer. Review the bank histories.',
  unavailable: 'Automatic posting check could not reach Plaid. Renew bank access or retry this check.',
})[match.status] || 'Plaid could not verify this transfer.'

export default function BankTransferQueue({ settingsTarget, openSettings, profilesMode = false, tenantId, checking, sources, basis, lastSavedCheck, lastServerCheck, cashPlan, providerLinked, serverOnline, serverChecking, onServerCheck, onReviewRepayment, repaymentEnabled, onAccountsDiscovered, onBalanceObserved, onBrowserCheckRecorded }: { settingsTarget: HTMLElement | null; openSettings: () => void; profilesMode?: boolean; tenantId: number; checking: Account[]; sources: Account[]; basis: string; lastSavedCheck: SavedCheck | null; lastServerCheck: ServerCheck | null; cashPlan: CashPlan | null; providerLinked: boolean; serverOnline: boolean; serverChecking: boolean; onServerCheck: () => void; onReviewRepayment: () => void; repaymentEnabled: boolean; onAccountsDiscovered: (accounts: Account[]) => Promise<MonitoredAccounts>; onBalanceObserved: (result: BalanceCheck) => void; onBrowserCheckRecorded: () => void }) {
  ...
  const [plaidMatches, setPlaidMatches] = useState<Record<string, PlaidMatch>>({})
  const [plaidMatchBusy, setPlaidMatchBusy] = useState<string | null>(null)
  ...
  const plaidPollRef = useRef<() => void>(() => undefined)
  const plaidPollState = useRef({ running: false, lastAttempt: new Map<string, number>() })
  ...
  const checkPlaidMatch = async (draft: Draft, confirm = false, automatic = false) => {
    const match = plaidMatches[draft.id]
    if (confirm && (!match?.source_evidence || !match.destination_evidence)) return
    setPlaidMatchBusy(draft.id); if (!automatic) { setError(''); setNotice('') }
    try {
      const response = await bankMonitorApi.draftPlaidMatch(tenantId, draft.id, confirm ? {
        confirm: true, source_evidence: match!.source_evidence,
        destination_evidence: match!.destination_evidence,
      } : { automatic })
      if (!active.current) return
      if (response.data.status === 'bank_history_matched') {
        await reload()
        setPlaidMatches(current => { const next = { ...current }; delete next[draft.id]; return next })
        setNotice(automatic ? 'Transfer matched automatically: its reference and amount appear in both posted bank entries.' : 'Both posted bank entries were confirmed from Plaid. This transfer is marked complete.')
      } else setPlaidMatches(current => ({ ...current, [draft.id]: response.data as PlaidMatch }))
    } catch (error: unknown) {
      if (active.current) {
        if (automatic) setPlaidMatches(current => ({ ...current, [draft.id]: { status: 'unavailable' } }))
        else setError(apiErrorMessage(error, 'Unable to check this transfer in Plaid.'))
      }
    } finally { if (active.current) setPlaidMatchBusy(null) }
  }
  plaidPollRef.current = () => {
    if (!providerLinked || busy || plaidMatchBusy || plaidPollState.current.running || document.visibilityState === 'hidden') return
    const draft = drafts.find(item => ['prepared_awaiting_submission', 'preparation_failed'].includes(item.status) && Date.now() - (plaidPollState.current.lastAttempt.get(item.id) || 0) >= 60_000)
    if (!draft) return
    plaidPollState.current.running = true
    plaidPollState.current.lastAttempt.set(draft.id, Date.now())
    void checkPlaidMatch(draft, false, true).finally(() => { plaidPollState.current.running = false })
  }
  useEffect(() => {
    const tick = () => plaidPollRef.current()
    tick()
  ...
  const cancelDraft = async (draft: Draft) => {
    setBusy(true); setError('')
    try { await bankMonitorApi.cancelDraft(tenantId, draft.id); await reload() }
    catch (error) { if (active.current) setError(apiErrorMessage(error, 'Unable to remove this draft.')) }
  ...
  const create = async (event: React.FormEvent) => {
    ...
    try {
      const amountCents = centsFromMoneyInput(amount)
      if (amountCents <= 0) throw new Error('Enter a full charge amount greater than zero.')
      ...
      await bankMonitorApi.createDraft(tenantId, { charge_reference: reference, amount_cents: amountCents, from_last4: from, to_last4: to, memo })
      if (!active.current) return
  ...
  const connect = async () => {
    ...
    try {
      ...
      if (active.current) { setExtension(candidate); setDraftExtension(candidate); setConnected(true); setConnectionState('connected'); setDetectedVersion(response.version || '') }
  ...
  const reloadAndReconnect = async () => {
    ...
    for (let attempt = 0; attempt < 30; attempt += 1) {
      ...
      try {
        const response = await send<ExtensionResponse>(extension, { type: 'ELIS_PING' })
        if (versionAtLeast(response.version, REQUIRED_EXTENSION_VERSION)) return response
  ...
  const reloadAssistant = async () => {
    ...
    try {
      const response = await reloadAndReconnect()
  ...
  const checkBalances = async (background = false) => {
    if (profilesMode) { if (!background) setError('Refresh the selected banking profile above.'); return }
    ...
    try {
      ...
      for (const destination of effectiveChecking) {
        ...
        for (const transaction of history.transactions) {
          ...
          const matchingDraft = existing.find(draft => matchesExistingDraft(draft, destination, transaction, chargeReference, transactionMemo))
          const effectiveDate = bankDateIso(transaction.date)
          if (matchingDraft && effectiveDate && (matchingDraft.bank_state !== (transaction.pending ? 'pending' : 'posted') || matchingDraft.bank_effective_date !== effectiveDate)) {
            await bankMonitorApi.updateDraftBankDetails(tenantId, matchingDraft.id, { bank_state: transaction.pending ? 'pending' : 'posted', bank_effective_date: effectiveDate })
          }
        }
        ...
        for (const transaction of candidates) {
          ...
          identified += 1
          try {
            await bankMonitorApi.createDraft(tenantId, { charge_reference: chargeReference, amount_cents: transaction.amount_cents, from_last4: sourceRow.source.last4, to_last4: destination.last4, memo: transactionMemo, bank_state: transaction.pending ? 'pending' : 'posted', bank_effective_date: effectiveDate })
            created += 1
  ...
  const prepare = async (draft: Draft) => {
    if (draft.bank_session && !versionAtLeast(detectedVersion, '0.1.24')) { setError('Reload the bank assistant to version 0.1.24 or newer for banking-profile preparation.'); return }
    const blockingDraft = drafts.find(item => item.id !== draft.id && item.status === 'prepared_awaiting_submission')
    if (blockingDraft) {
      setError(`Finish checking the ${money(blockingDraft.amount_cents)} transfer before preparing another one.`)
      return
    }
    ...
    try {
      ...
      if (active.current) { setNotice(restored ? `${errorMessage(error, 'Preparation stopped before the bank form opened.')} The draft is ready to resume.` : ''); if (!restored) setError(errorMessage(error, 'Preparation failed. Review the bank tab before continuing.')) }
    } finally { if (active.current) { await reload().catch(() => undefined); setBusy(false); setOperation(null) } }
  }
  const retryPreparation = async (draft: Draft) => {
    const blockingDraft = drafts.find(item => item.id !== draft.id && item.status === 'prepared_awaiting_submission')
    if (blockingDraft) {
      setError(`Finish checking the ${money(blockingDraft.amount_cents)} transfer before resetting this interrupted draft.`)
      return
    }
    setBusy(true); setError(''); setNotice(''); setOperation({ message: 'Resetting the interrupted bank preparation…', started: Date.now() })
    try {
      if (!versionAtLeast(detectedVersion, SELF_RELOAD_VERSION)) throw new Error('Reload the bank assistant from chrome://extensions before resetting this draft.')
      const response = await reloadAndReconnect()
      if (active.current) { setDetectedVersion(response.version || ''); setConnected(true); setConnectionState('connected') }
      await bankMonitorApi.retryDraft(tenantId,draft.id)
      await reload()
      if (active.current) setNotice('The stale preparation lock was cleared. This draft is ready to prepare again.')
  ...
  useEffect(() => {
    const checkAfterReturn = () => {
      ...
      if (providerLinked || busy || !connected || automaticVerificationRunning.current) return
  ...
  }, [busy, connected, drafts, providerLinked])

  const savedAccounts = lastSavedCheck?.result.accounts && lastSavedCheck.result.credit_accounts ? [
    ...lastSavedCheck.result.accounts.map(account => ({ ...account, available_credit_cents: null, outstanding_cents: null })),
    ...lastSavedCheck.result.credit_accounts.map(account => ({ last4: account.last4, current_cents: null, available_cents: null, available_credit_cents: account.available_credit_cents, outstanding_cents: null })),
  ] : null
  ...
  const missingCash = checkedRows.some(row => row.balance?.current_cents == null || row.balance.available_cents == null)
  const unavailableCash = checkedRows.some(row => row.balance?.available_cents != null && row.balance.available_cents <= 0)
  ...
  const pendingConfirmation = drafts.find(draft => ['prepared_awaiting_submission', 'preparation_failed'].includes(draft.status))
  const connectionText = connected ? 'Chrome assistant ready for detailed bank review' : connectionState === 'checking' ? 'Checking the saved Chrome assistant…' : connectionState === 'outdated' ? `Chrome assistant update required · ${detectedVersion} loaded` : extension ? 'Chrome assistant unavailable for detailed bank review' : 'Chrome assistant needed for detailed bank review'

  return <div className="space-y-5">
    {!profilesMode && <section className="overflow-hidden rounded-2xl bg-slate-950 text-white shadow-sm ring-1 ring-slate-900/10">
      <div className="p-5 sm:p-7">
        <div className="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between">
          <div className="min-w-0">
            ...
            {!displayedCheck ? <><h2 className="text-2xl font-semibold tracking-tight sm:text-3xl">Are your checking accounts covered?</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-300">Read every configured account, then review cash availability and transfer possibilities.</p></> : <div className="flex items-start gap-3"><span className={`mt-1 grid h-9 w-9 shrink-0 place-items-center rounded-full ${negativePosted || unavailableCash || missingCash ? 'bg-amber-500/15 text-amber-300' : 'bg-emerald-500/15 text-emerald-300'}`}><Icon name={negativePosted || unavailableCash || missingCash ? 'alert' : 'check'} /></span><div><h2 className="text-2xl font-semibold tracking-tight sm:text-3xl">{missingCash ? 'Some checking balances are unavailable' : negativePosted ? `${money(negativePosted)} negative posted balance` : unavailableCash ? 'Checking cash needs review' : displayedCheck.source === 'Plaid server' ? 'Posted balances are positive' : 'Checking accounts are positive'}</h2><p className="mt-2 text-sm text-slate-300">{displayedCheck.source} balance read {new Date(displayedCheck.checked_at).toLocaleString()} · {displayedCheck.source === 'Plaid server' ? 'Pending debits not fully verified' : 'Bank histories reviewed'} · No transfer submitted.</p></div></div>}
          </div>
          <div className="flex w-full shrink-0 flex-col gap-2 sm:w-auto"><button type="button" disabled={providerLinked ? !serverOnline || serverChecking : busy || !connected} onClick={() => providerLinked ? onServerCheck() : void checkBalances()} className="inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-xl bg-blue-500 px-5 py-3 font-semibold text-white shadow-sm transition-[background-color,transform] duration-150 hover:bg-blue-400 active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-45 motion-reduce:transition-none focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-300 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-950"><Icon name="refresh" className={`h-4 w-4 ${serverChecking || operation?.message.startsWith('Reading balances') ? 'animate-spin motion-reduce:animate-none' : ''}`} />{providerLinked ? serverChecking ? 'Refreshing…' : 'Refresh via Plaid' : operation?.message.startsWith('Reading balances') ? 'Checking bank…' : 'Check bank now'}</button>{providerLinked && <button type="button" disabled={profilesMode || busy || !connected} onClick={() => void checkBalances()} className="min-h-11 rounded-xl border border-white/25 px-4 text-sm font-semibold text-white hover:bg-white/10 disabled:cursor-not-allowed disabled:opacity-45">Review Truliant history</button>}</div>
        </div>
        {displayedCheck && !cashPlan && <div className="mt-7 grid gap-6 lg:grid-cols-2">
          <section aria-labelledby="cash-accounts-heading">
            <div className="mb-3 flex items-center justify-between gap-3"><div className="flex items-center gap-2"><span className="grid h-8 w-8 place-items-center rounded-lg bg-emerald-400/15 text-emerald-300"><Icon name="wallet" className="h-4 w-4" /></span><h3 id="cash-accounts-heading" className="text-sm font-semibold text-emerald-100">Cash accounts</h3></div><span className="text-xs text-emerald-200/70">Posted and bank-available balances</span></div>
          ...
          <section aria-labelledby="credit-lines-heading">
            <div className="mb-3 flex items-center justify-between gap-3"><div className="flex items-center gap-2"><span className="grid h-8 w-8 place-items-center rounded-lg bg-blue-400/15 text-blue-300"><Icon name="credit" className="h-4 w-4" /></span><h3 id="credit-lines-heading" className="text-sm font-semibold text-blue-100">Credit lines</h3></div><span className="text-xs text-blue-200/70">Borrowing capacity and payoff</span></div>
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-1 2xl:grid-cols-2">{fundingRows.map(({ account, balance }) => <article key={account.last4} className="min-h-44 rounded-2xl bg-blue-950/55 p-5 ring-1 ring-inset ring-blue-400/20"><div className="flex items-start justify-between gap-3"><span className="rounded-lg bg-blue-400/15 px-2.5 py-1 text-xs font-semibold text-blue-200">Credit line</span><span className="text-sm font-medium tabular-nums text-blue-200/75">••{account.last4}</span></div><h4 className="mt-4 min-h-10 text-sm font-medium leading-5 text-blue-50">{account.nickname}</h4><p className="mt-3 text-2xl font-semibold tracking-tight tabular-nums text-white">{balance?.available_credit_cents == null ? 'Unavailable' : money(balance.available_credit_cents)}</p><div className="mt-4 flex items-center justify-between gap-3 border-t border-blue-300/15 pt-3 text-xs"><span className="text-blue-100/65">Available credit</span><span className="font-medium tabular-nums text-blue-100">{balance?.outstanding_cents == null ? 'Balance owed unknown' : `${money(balance.outstanding_cents)} balance owed`}</span></div></article>)}</div>
          </section>
        </div>}
      </div>
      <div className="flex flex-wrap gap-x-6 gap-y-2 border-t border-white/10 bg-white/[0.03] px-5 py-3 text-sm sm:px-7">{providerLinked && <div className="flex items-center gap-2"><span className={`h-2 w-2 rounded-full ${serverOnline ? 'bg-emerald-400' : 'bg-amber-400'}`} /><span className="text-slate-300">{serverOnline ? 'Plaid server read available' : 'Plaid connection saved · worker offline'}</span></div>}<div className="flex items-center gap-2"><span className={`h-2 w-2 rounded-full ${connected ? 'bg-emerald-400' : connectionState === 'checking' ? 'animate-pulse bg-blue-400 motion-reduce:animate-none' : 'bg-amber-400'}`} /><span className="text-slate-300">{connectionText}</span></div></div>
    </section>}

    {!profilesMode && cashPlan && <BankCashPlan plan={cashPlan} creditAccounts={sources.map(account => ({ ...account, balance: displayedCheck?.accounts.find(item => item.last4 === account.last4) }))} balanceObservedAt={displayedCheck?.checked_at || null} providerAccountLast4={lastServerCheck?.accounts.map(account => account.last4) || []} repaymentEnabled={repaymentEnabled} onReviewRepayment={onReviewRepayment} />}
    ...
    {!connected && <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm"><p>{connectionState === 'outdated' ? 'Update the bank assistant to prepare transfers.' : 'Connect the bank assistant to prepare transfers.'}</p><button type="button" onClick={openSettings} className="font-semibold text-blue-700">Open Monitoring settings</button></div>}
    {settingsTarget && !connected && extension && connectionState !== 'checking' && createPortal(<section className="flex flex-col gap-4 rounded-2xl border border-amber-200 bg-amber-50 p-5 shadow-sm sm:flex-row sm:items-center sm:justify-between"><div className="flex items-start gap-3"><span className="grid h-9 w-9 shrink-0 place-items-center rounded-lg bg-white text-amber-700 ring-1 ring-amber-200"><Icon name="refresh" /></span><div><h2 className="font-semibold text-amber-950">{connectionState === 'outdated' ? 'Reload the saved bank assistant' : 'Bank assistant did not respond'}</h2><p className="mt-1 text-sm leading-6 text-amber-900">{connectionState === 'outdated' ? versionAtLeast(detectedVersion, SELF_RELOAD_VERSION) ? `Chrome has version ${detectedVersion}; ELIS needs ${REQUIRED_EXTENSION_VERSION}. Reload it here without changing the saved extension ID or accounts.` : `Chrome still has version ${detectedVersion}. Open chrome://extensions, reload ELIS Bank Form Assistant once, then return here. Your extension ID and imported accounts remain saved.` : 'The saved extension ID and account setup remain intact. Chrome may still be restarting the extension.'}</p></div></div><div className="flex shrink-0 gap-2"><button type="button" disabled={busy} onClick={() => connectionState === 'outdated' ? void reloadAssistant() : void ping()} className="min-h-11 rounded-xl bg-amber-900 px-4 text-sm font-semibold text-white hover:bg-amber-800 disabled:opacity-50">{connectionState === 'outdated' ? versionAtLeast(detectedVersion, SELF_RELOAD_VERSION) ? 'Reload assistant now' : 'Check after reload' : 'Try reconnecting'}</button><button type="button" onClick={openSettings} className="min-h-11 rounded-xl px-3 text-sm font-semibold text-amber-900 hover:bg-amber-100">Change extension</button></div></section>, settingsTarget)}

    {settingsTarget && createPortal(<section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-start gap-3"><span className="grid h-9 w-9 shrink-0 place-items-center rounded-lg bg-blue-50 text-blue-700"><Icon name="link" /></span><div><h2 className="font-semibold text-slate-950">{extension ? 'Change bank assistant' : 'Connect bank assistant'}</h2>{error && <p role="alert" className="mt-2 text-sm text-red-800">{error}</p>}<p className="mt-1 text-sm text-slate-600">The extension ID is saved in this Chrome profile. Account import runs only when you connect a new extension.</p></div></div><label className="mt-4 block text-sm font-medium text-slate-700">Chrome extension ID<input className="mt-2 block min-h-11 w-full rounded-xl border border-slate-300 px-3 text-slate-950 outline-none transition focus:border-blue-500 focus:ring-4 focus:ring-blue-100" value={draftExtension} maxLength={32} onChange={e => setDraftExtension(e.target.value.trim())} placeholder="32-letter extension ID" /></label><div className="mt-4 flex flex-col gap-2 sm:flex-row"><button type="button" disabled={busy || !/^[a-p]{32}$/.test(draftExtension)} onClick={connect} className="inline-flex min-h-11 items-center justify-center rounded-xl bg-blue-700 px-4 font-semibold text-white transition hover:bg-blue-800 active:scale-[0.98] disabled:opacity-45 motion-reduce:transition-none">{busy ? 'Connecting…' : extension ? 'Save replacement' : 'Connect and import'}</button>{extension && <button type="button" disabled={busy} onClick={() => { setDraftExtension(extension); setError('') }} className="min-h-11 rounded-xl px-4 font-medium text-slate-700 hover:bg-slate-100">Cancel</button>}</div></section>, settingsTarget)}

    <section className="rounded-2xl border border-slate-200 bg-white shadow-sm">
      <div className="flex flex-col gap-3 border-b border-slate-200 p-5 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-xs font-semibold uppercase tracking-wider text-slate-500">Action queue</p><h2 className="mt-1 text-xl font-semibold tracking-tight text-slate-950">Transfer queue</h2><p className="mt-1 text-sm text-slate-600">{actionableDrafts.length ? `${actionableDrafts.length} transfer${actionableDrafts.length === 1 ? '' : 's'} need attention.` : 'No transfers need attention.'}</p></div><button type="button" disabled={profilesMode || busy || !connected} onClick={discoverAccounts} className="min-h-11 self-start rounded-xl px-3 text-sm font-semibold text-blue-700 hover:bg-blue-50 disabled:cursor-not-allowed disabled:text-blue-300 disabled:hover:bg-transparent">Refresh accounts</button></div>
      <div className="divide-y divide-slate-200">{actionableDrafts.length === 0 && <div className="p-8 text-center"><span className="mx-auto grid h-10 w-10 place-items-center rounded-full bg-emerald-50 text-emerald-700"><Icon name="check" /></span><p className="mt-3 font-medium text-slate-900">Queue is clear</p><p className="mt-1 text-sm text-slate-600">New reviewed charges will appear here.</p></div>}{actionableDrafts.map(draft => { const match = plaidMatches[draft.id]; const baseMeta = statusMeta(draft.status); const meta = waitingForPosting.has(draft.id) && baseMeta.action === 'verify' ? { ...baseMeta, label: 'Waiting for bank posting' } : baseMeta; const blockedByActiveTransfer = pendingConfirmation && pendingConfirmation.id !== draft.id; return <article key={draft.id} className="grid gap-4 p-5 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><p className="text-lg font-semibold tabular-nums text-slate-950">{money(draft.amount_cents)}</p><span className={`rounded-full px-2.5 py-1 text-xs font-semibold ring-1 ring-inset ${meta.tone}`}>{meta.label}</span>{draft.bank_state && draft.bank_effective_date && <span className={`rounded-full px-2.5 py-1 text-xs font-medium ${draft.bank_state === 'pending' ? 'bg-violet-50 text-violet-800 ring-1 ring-inset ring-violet-200' : 'bg-slate-100 text-slate-700'}`}>{draft.bank_state === 'pending' ? 'Expected to post' : 'Posted'} {bankDateLabel(draft.bank_effective_date)}</span>}</div><p className="mt-1 truncate font-medium text-slate-800">{draft.memo}</p>{draft.bank_session && <p className="mt-2 text-sm font-semibold text-blue-800">Required login: {draft.bank_session.profile_name}</p>}{providerLinked && draft.status === 'prepared_awaiting_submission' && <p className="mt-2 text-xs text-blue-700">ELIS checks posting automatically while this page is open.</p>}<p className="mt-1 text-sm text-slate-500">••{draft.from_last4} <span aria-hidden="true">→</span> ••{draft.to_last4}</p>{draft.status === 'reviewed' && (draft.charge_reference.startsWith('bank-review-') || draft.charge_reference.startsWith('bank-profile-')) && <button type="button" disabled={busy} onClick={() => void cancelDraft(draft)} className="mt-2 min-h-11 rounded-lg px-2 text-xs font-semibold text-slate-600 hover:bg-slate-100">Remove unprepared draft</button>}</div>{meta.action === 'prepare' && (blockedByActiveTransfer ? <div className="max-w-xs text-right"><button type="button" disabled className="min-h-11 rounded-xl border border-slate-200 bg-slate-100 px-4 font-semibold text-slate-500">Waiting for active transfer</button><p className="mt-2 text-xs leading-5 text-slate-500">Finish the {money(pendingConfirmation.amount_cents)} transfer status check first.</p></div> : <button disabled={busy || !connected} onClick={() => prepare(draft)} className="inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-xl bg-blue-700 px-4 font-semibold text-white transition hover:bg-blue-800 active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-45 motion-reduce:transition-none sm:w-auto">Prepare in Truliant <Icon name="arrow" className="h-4 w-4" /></button>)}{meta.action === 'verify' && <button disabled={busy || (providerLinked ? plaidMatchBusy !== null : !connected)} onClick={() => providerLinked ? void checkPlaidMatch(draft) : void verify(draft)} className="inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-xl border border-slate-300 bg-white px-4 font-semibold text-slate-800 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-45 sm:w-auto">{providerLinked ? plaidMatchBusy === draft.id ? 'Checking posting…' : 'Refresh posting status' : waitingForPosting.has(draft.id) ? 'Check posting now' : 'Check transfer status'} <Icon name="arrow" className="h-4 w-4" /></button>}{meta.action === 'retry' && <div className="max-w-xs space-y-2 text-right"><button type="button" disabled={busy || (providerLinked ? plaidMatchBusy !== null : !connected)} onClick={() => providerLinked ? void checkPlaidMatch(draft) : void verify(draft)} className="min-h-11 rounded-xl border border-slate-300 bg-white px-4 font-semibold text-slate-800 disabled:opacity-45">{plaidMatchBusy === draft.id ? 'Checking posting…' : 'Check submitted transfer'}</button><p className="text-xs leading-5 text-slate-600">Already submitted in Truliant? Check posting here. An edited memo may require confirmation of both bank entries.</p><button disabled={busy || !connected || Boolean(blockedByActiveTransfer)} onClick={() => retryPreparation(draft)} className="min-h-11 rounded-xl border border-amber-300 bg-amber-50 px-4 font-semibold text-amber-900 hover:bg-amber-100 disabled:cursor-not-allowed disabled:border-slate-200 disabled:bg-slate-100 disabled:text-slate-500">{blockedByActiveTransfer ? 'Waiting for active transfer' : 'Reset interrupted preparation'}</button><p className="mt-2 text-xs leading-5 text-slate-500">{blockedByActiveTransfer ? `Finish the ${money(pendingConfirmation.amount_cents)} transfer status check first.` : 'Use after confirming no transfer was submitted in Truliant.'}</p></div>}          {match && <div className="rounded-xl bg-slate-50 p-4 text-sm sm:col-span-2" aria-live="polite">{match.status === 'ready_for_confirmation' && match.source && match.destination ? <><p className="font-semibold text-slate-950">Both posted entries found. Confirm they represent this transfer:</p><div className="mt-3 grid gap-3 sm:grid-cols-2"><p className="rounded-lg bg-white p-3 text-slate-700"><strong className="block text-slate-900">Debit · ••{draft.from_last4}</strong>{match.source.description} · {match.source.date} · {money(match.source.amount_cents)}</p><p className="rounded-lg bg-white p-3 text-slate-700"><strong className="block text-slate-900">Credit · ••{draft.to_last4}</strong>{match.destination.description} · {match.destination.date} · {money(-match.destination.amount_cents)}</p></div><button type="button" disabled={plaidMatchBusy !== null} onClick={() => void checkPlaidMatch(draft, true)} className="mt-3 min-h-11 rounded-xl bg-emerald-700 px-4 font-semibold text-white hover:bg-emerald-800 disabled:opacity-45">Confirm posted pair</button></> : <p className="text-slate-700">{plaidMatchMessage(match)}</p>}</div>}</article> })}</div>
