...
export default function BankTransferQueue({ tenantId, checking, sources, basis, onAccountsDiscovered }: { tenantId: number; checking: Account[]; sources: Account[]; basis: string; onAccountsDiscovered: (accounts: Account[]) => void }) {
  ...
  return <div className="space-y-5">
    <section className="overflow-hidden rounded-2xl bg-slate-950 text-white shadow-sm ring-1 ring-slate-900/10">
      ...
      <div className="flex flex-col gap-3 border-t border-white/10 bg-white/[0.03] px-5 py-3 text-sm sm:flex-row sm:items-center sm:justify-between sm:px-7"><div className="flex items-center gap-2"><span className={`h-2 w-2 rounded-full ${connected ? 'bg-emerald-400' : connectionState === 'checking' ? 'animate-pulse bg-blue-400 motion-reduce:animate-none' : 'bg-amber-400'}`} /><span className="text-slate-300">{connectionText}</span></div>{!connected && connectionState !== 'checking' && <button type="button" onClick={() => void ping()} className="min-h-11 self-start font-medium text-blue-300 hover:text-blue-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-300 sm:min-h-0">Check again</button>}</div>
    </section>
