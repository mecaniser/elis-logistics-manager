...
export default function BankCashPlan({ plan, creditAccounts, balanceObservedAt, providerAccountLast4, onReviewRepayment }: { plan: CashPlan; creditAccounts: CreditAccount[]; balanceObservedAt: string | null; providerAccountLast4: string[]; repaymentEnabled: boolean; onReviewRepayment: () => void }) {
  ...
  return <section aria-labelledby="cash-plan-title" className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
    ...
    <div className="border-t border-slate-200 p-5 sm:p-6">
      ...
      {plan.checking_routes.length === 0 && plan.credit_routes.length === 0 ? <p className="mt-4 rounded-xl bg-slate-50 p-4 text-sm text-slate-600">No balance-based route is available with the current account balances and reserve.</p> : <div className="mt-4 space-y-2">
        {plan.checking_routes.map((route, index) => <div key={`checking-${index}`} className="rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-sm"><div className="flex flex-wrap items-start justify-between gap-2"><span className="text-xs font-semibold uppercase tracking-wide text-emerald-800">Step {index + 1} · Checking coverage</span><strong className="tabular-nums text-emerald-950">Up to {money(route.amount_cents)}</strong></div><p className="mt-2 font-medium text-emerald-950">From checking {cashNames[route.from_last4] || 'account'} · ••{route.from_last4}</p><p className="text-emerald-950">To checking {cashNames[route.to_last4] || 'account'} · ••{route.to_last4}</p><p className="mt-2 text-xs text-amber-900">Review transfers to calculate this coverage from fresh balances.</p></div>)}
        {plan.credit_routes.map((route, index) => <div key={`credit-${index}`} className="rounded-xl border border-blue-200 bg-blue-50 p-4 text-sm"><div className="flex flex-wrap items-start justify-between gap-2"><span className="text-xs font-semibold uppercase tracking-wide text-blue-800">Step {plan.checking_routes.length + index + 1} · Credit repayment</span><strong className="tabular-nums text-blue-950">Up to {money(route.amount_cents)}</strong></div><p className="mt-2 font-medium text-blue-950">From checking {cashNames[route.from_last4] || 'account'} · ••{route.from_last4}</p><p className="text-blue-950">To credit {creditNames[route.to_last4] || 'account'} · ••{route.to_last4}</p><p className="mt-2 text-xs text-amber-900">Fresh balances and unfinished drafts are checked in transfer review.</p></div>)}
      </div>}
      <p className="mt-4 text-xs leading-5 text-slate-500">Review the prefilled amounts and create drafts, then let ELIS prepare the Truliant form. You sign in and submit the transfer yourself.</p>
    </div>
  </section>
}
