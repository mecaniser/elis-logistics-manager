...
export default function Finance() {
  ...
  return <div className="finance-app">
    ...
    {data && r && currentScope && !error && <>
      ...
      {section === 'money' && <><CashView cash={r.owner_cash} /><OwnerReimbursements key={`owner-reimbursements-${currentTenant?.id}`} /><section className="finance-section"><h2>Transactions to match</h2><DataTable rows={data.workspace.transactions.filter(t => !t.matched)} columns={['date', 'description', 'amount', 'id']} /><h2>Outstanding obligations</h2><DataTable rows={data.workspace.claims.filter(c => Number(c.remaining) > 0)} columns={['description', 'creditor', 'remaining', 'source_ref']} /><h2>Protected cash</h2><DataTable rows={data.workspace.reserves} columns={['pair_id', 'purpose', 'balance', 'note']} /></section></>}
