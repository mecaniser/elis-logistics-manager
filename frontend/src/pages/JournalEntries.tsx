...
export default function JournalEntries() {
  ...
  const handleExport = async (format: 'csv' | 'excel') => {
    try {
      const response = await accountingApi.exportJournalEntries(
        format,
        startDate || undefined,
        endDate || undefined,
        referenceTypeFilter || undefined,
      ...
      a.href = url
      a.download = `journal_entries_${new Date().toISOString().split('T')[0]}.${format === 'excel' ? 'xlsx' : 'csv'}`
              ...
              <div key={entry.id} className="bg-white rounded-lg shadow overflow-hidden">
                <div className="px-4 sm:px-6 py-3 sm:py-4 bg-gray-50 border-b">
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 sm:gap-4">
                    <div className="flex-1 min-w-0">
                      ...
                      {entry.description && (
                        <p className="text-xs sm:text-sm text-gray-600 mt-1 break-words">{entry.description}</p>
                      )}
                    </div>
                    <div className="text-left sm:text-right flex-shrink-0">
                      <div className="flex sm:flex-col gap-3 sm:gap-0">
                        ...
                        <div>
                          <div className="text-xs sm:text-sm text-gray-500 sm:mt-2">Total Credits</div>
                ...
                <div className="lg:hidden divide-y divide-gray-200">
                  {entry.lines.map((line) => (
                    <div key={line.id} className="px-4 py-3 hover:bg-gray-50">
                      <div className="space-y-2">
                        <div className="flex justify-between items-start gap-2">
                          <div className="flex-1 min-w-0">
                            <div className="text-xs font-medium text-gray-500 uppercase mb-1">Account</div>
                            <div className="text-sm text-gray-900 break-words">{getAccountName(line.account_id)}</div>
                          </div>
                        </div>
                        ...
                        {line.description && (
                          <div>
                            ...
                            <div className="text-sm text-gray-500 break-words">{line.description}</div>
                          </div>
                        )}
                      </div>
                    </div>
                      ...
                      {entry.lines.map((line) => (
                        <tr key={line.id} className="hover:bg-gray-50">
                          <td className="px-6 py-4 text-sm text-gray-900">
                            <span className="break-words">{getAccountName(line.account_id)}</span>
                          </td>
