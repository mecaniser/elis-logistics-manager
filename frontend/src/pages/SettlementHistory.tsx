...
import { vehicleIdentities } from '../components/finance/vehicleIdentity'
...
export default function SettlementHistory() {
  ...
  return <div className="finance-app owner-dashboard"><header className="finance-header"><div><h1>Settlement reconciliation</h1><p className="finance-help">Follow every original from freight and driver pay to fuel, deductions and payout.</p></div><Link className="finance-secondary" to="/settlements">Back to settlements</Link></header>
    ...
    {report&&report.tenant_id===currentTenant?.id&&<>
      ...
      <section className="owner-panel" id="review-records"><h2>Review each settlement</h2><p className="finance-help">If you have an original PDF for a saved record, open that record and attach it. We preserve the PDF, compare it with the saved amounts and flag differences. A missing original can remain visible while you search for it; no amount is assumed to be zero.</p><div className="owner-filters"><label>Equipment<select value={asset} onChange={e=>setAsset(e.target.value)}><option value="all">All equipment</option>{[...identities].map(([id,identity])=><option key={id} value={id}>{identity.label} — {identity.currentName}</option>)}</select></label><label>Review status<select value={status} onChange={e=>setStatus(e.target.value)}><option value="all">All records</option><option value="cost">Cost and amount exceptions</option><option value="arithmetic_matched">Arithmetic matched</option><option value="needs_review">Mapping or source review</option><option value="missing_source">Original unavailable</option></select></label><label>Find a statement<input type="search" value={query} onChange={e=>setQuery(e.target.value)} placeholder="VIN, name, date, carrier…" /></label></div><p className="finance-help">Showing {Math.min(visibleCount, rows.length)} of {rows.length} matching records. Expand a record for reconciliation and supporting rows.</p>
      ...
      {rows.slice(0,visibleCount).map(r=><details className="history-record" key={r.id} open={params.get('id')===String(r.id)&&query===params.get('id') ? true : undefined}><summary><span><strong title={identities.get(r.asset_id)?.vin}>{identities.get(r.asset_id)?.label}</strong><small>Current: {r.name}</small><small>{r.date} · {r.source_ref||r.provider}</small></span><span><small>Statement payout</small><strong>{dollars(r.remainder)}</strong></span><span className={`finance-status ${r.status==='arithmetic_matched'?'is-ready':''}`}>{r.status==='arithmetic_matched'?'Arithmetic matched':r.status==='missing_source'?'Original unavailable':'Needs review'}</span><svg className="history-chevron" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden="true"><path d="m9 5 7 7-7 7" /></svg></summary>
        <div className="history-body"><p className="finance-help">VIN: {r.vin || 'Not recorded'}. Current vehicle name: {r.name}. This name may differ from the unit or driver on the original statement.</p><p className="finance-help">{r.basis==='source_statement'?'Amounts extracted from the original PDF.':'Stored amounts only; source mapping has not been verified.'} Deposit matching and driver payment verification remain open.</p>
