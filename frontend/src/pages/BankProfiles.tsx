...
export default function BankProfiles({ tenantId, onMode, onDraft }: { tenantId: number; onMode: (value: boolean) => void; onDraft: () => void }) {
  ...
  return <section aria-label="Banking profiles" className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
    ...
    {!data.profiles.length ? <div><button disabled={busy} onClick={() => void request('/initialize')} className={`${primary} mt-4`}>{busy ? 'Loading…' : 'Set up banking profiles'}</button><button disabled={busy} onClick={() => void connect()} className={`${button} ml-2 mt-4`}>Connect a bank login</button></div> : <>
      ...
      <details className="mt-4 rounded-xl border border-slate-200 p-4"><summary className="cursor-pointer text-sm font-semibold text-slate-800">Manage bank connections</summary><div className="mt-4 space-y-3"><label className="block text-sm text-slate-700">New banking profile name<input value={name} maxLength={80} onChange={e => setName(e.target.value)} className="mt-1 block min-h-11 w-full rounded-xl border border-slate-300 px-3" /></label><p className="text-xs leading-5 text-slate-500">For ••3304 → ••8264, sign in with the separate Main Business credentials. Adding a profile preserves existing connections. Other institutions can be monitored; form preparation currently supports Truliant only.</p><button disabled={busy || !name.trim()} onClick={() => void connect()} className={primary}>Connect another bank login</button></div></details>
      {active && <>
        <div className="mt-5 flex flex-wrap items-center justify-between gap-3"><p className="text-sm text-slate-600">{active.status === 'synced' ? 'Synced' : active.status.replace(/_/g, ' ')}{active.last_checked_at && ` · Last successful read ${new Date(active.last_checked_at).toLocaleString()}`}</p><div className="flex gap-2"><button disabled={busy} onClick={() => { setReview(null); void request(`/${active.id}/sync`) }} className={primary}>{busy ? 'Working…' : 'Refresh profile'}</button><button disabled={busy} onClick={() => void connect(active)} className={button}>Renew access</button></div></div>
